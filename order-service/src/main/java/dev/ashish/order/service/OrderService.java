...
public class OrderService {
	...
	public Order placeOrder(String customerId, String item, int quantity, BigDecimal amount) {
		...
		steps.save(new OrderStep(order.getId(), order.getStatus(), "order taken"));
	...
	@Transactional(readOnly = true)
	public List<OrderStep> stepsFor(UUID orderId) {
		return steps.findByOrderIdOrderByAtAsc(orderId);
	}
